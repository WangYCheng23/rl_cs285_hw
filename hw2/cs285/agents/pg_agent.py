...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        train_log = self.actor.update(observations, actions, qvals, advantages)
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        return np.array(list(itertools.chain(*q_values)))
    ...
    def estimate_advantage(self, obs: np.ndarray, rews_list: np.ndarray, q_values: np.ndarray, terminals: np.ndarray):
        ...
        if self.nn_baseline:
            ...
            batch_q_mean = np.mean(q_values)
            batch_q_std = np.std(q_values)
            values = q_values-(values_unnormalized-np.mean(values_unnormalized))/(np.std(values_unnormalized))*batch_q_std+batch_q_mean
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    ## HINT: use terminals to handle edge cases. terminals[i]
                        ...
                        ## 0 otherwise.
                    if terminals[i]==0:
                        advantages[i] = values[i]+self.gamma*self.gae_lambda*advantages[i+1]
                    else:
    ...
    def _discounted_return(self, rewards):
        ...
        """
        T = len(rewards)
        gamma_vector = self.gamma ** np.arange(T)
        discounted_rewards = gamma_vector * rewards
        list_of_discounted_returns = [sum(discounted_rewards) for _ in discounted_rewards]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        gamma_vector = self.gamma ** np.arange(T)
        discounted_rewards = gamma_vector * rewards
        compensate_gamma_vector = np.reciprocal(gamma_vector)
        # sum_{t'=t}^T gamma^(t') * r_{t'} / gamma^(t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
        list_of_discounted_cumsums = (np.cumsum(discounted_rewards[::-1])[::-1]*compensate_gamma_vector)
